...
@endsection
@section('content')
<form id="myForm" action="{{url('guru/kelas/'.$id_kelas_sekarang.'/kuis/buat/do')}}" onsubmit="saveForm()" method="POST" name="myForm">
    @csrf
                ...
                <div class="w-auto md:w-24 md:sticky md:top-4 bg-white dark:bg-ocean-light dark:bg-opacity-50 shadow-md rounded-md flex flex-row p-5 flex flex-col bg-opacity-50 backdrop-filter backdrop-blur p-2 lg:p-5 my-2 h-44 items-center">
                    ...
                    <div class="flex md:flex-col flex-wrap gap-1 h-min my-2">
                        <div x-data="{ dropdownOpen: true }" class="relative z-50">
                            <button type="button" onclick="openDropDown()" class="h-12 w-12 text-white rounded-lg bg-secondary-red hover:bg-secondary-red-hover dark:bg-secondary-red-hover dark:hover:bg-secondary-red shadow-lg block md:inline-block rounded-full text-3xl mr-auto">
                            ...
                            <div id="dropDown" class="absolute right-0 mt-2 w-32 bg-white rounded-md overflow-hidden hidden shadow-xl z-50">
                                <button type="button" class="block px-4 py-2 text-sm text-gray-800 border-b hover:bg-gray-200 z-50 w-full" onclick="pilihanGandaAdd()">Pilihan Ganda</button>
                                <button type="button" class="block px-4 py-2 text-sm text-gray-800 border-b hover:bg-gray-200 z-50 w-full" onclick="uraianAdd()">Uraian</button>
                            </div>
                        </div>
                            <button type="submit" class="h-12 w-12 text-white rounded-lg bg-secondary-red hover:bg-secondary-red-hover dark:bg-secondary-red-hover dark:hover:bg-secondary-red shadow-lg block md:inline-block rounded-full text-3xl mr-auto"><img src="{{url('/asset/save.png')}}" class="mx-auto" alt=""></button>
                    </div>
                </div>
                <div id="kuisPertanyaan" onload="loadOld()" class="flex-grow md:ml-2 z-40">
    ...
    <script>
        document.getElementById('kuisPertanyaan').innerHTML =  JSON.parse(localStorage.getItem('items')) ;
        let i = 0;
        ...
        function saveForm() {
            var htmlContents = document.getElementById('kuisPertanyaan').innerHTML;
            localStorage.setItem('items', JSON.stringify(htmlContents));
            console.log('saved');
        }
        function loadOld() {
