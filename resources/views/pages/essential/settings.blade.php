    ...
    <div class="bg-white dark:text-black dark:bg-ocean-light dark:bg-opacity-75 px-5 w-full lg:w-1/2 shadow-md rounded-md py-3 backdrop-filter backdrop-blur flex flex-col gap-2 my-2 items-center w-auto">
        <form action="/settings/update" method="post" class="flex flex-col my-4 xl:w-1/2 w-full gap-2">
            ...
            <div class="flex flex-col">
                ...
                <input type="password" placeholder="************" name="pengguna_password" class="border rounded-lg py-3 px-3 border-black dark:bg-ocean-light dark:bg-opacity-100" value="{{Auth::guard('satpam_pengguna')->user()->pengguna_password}}">
            </div>
            <div class="flex flex-col">
                <label class="font-bold text-lg">Peran</label>
                <input type="text" placeholder="" class="border rounded-lg py-3 px-3 border-black dark:bg-ocean-light dark:bg-opacity-100" value="{{Auth::guard('satpam_pengguna')->user()->pengguna_role==0?"Guru":"Murid"}}" readonly>
            </div>
